...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['organizer'])) {
        ...
        if ($events) {
            ...
            header('Content-Type: application/json');
            echo json_encode(array("status" => "error no result ", "message" => "No events found for the given organizer."));
        }
        ...
        header('Content-Type: application/json');
        echo json_encode(array("status" => "error oragnizer", "message" => "Missing 'organizer' parameter."));
    }
} else {
    // If the request method is not POST, return an error message
    header('Content-Type: application/json');
    echo json_encode(array("status" => "error post", "message" => "Invalid request method."));
}
?>
